	...
	fileComments
		.filter(blockComment => {
			...
			return (
				...
				tags &&
				comment.startsWith("/**") &&
				(meta.code.type === "MethodDefinition" || meta.code.type === "FunctionExpression" || meta.code.type === "FunctionDeclaration" || meta.code.type === 'ArrowFunctionExpression') &&
		...
		.forEach(blockComment => {
			...
			let apiBodyObj;
			try {
				apiBodyObj = JSON.parse(apiBodyStr);
			} catch (err) {}
